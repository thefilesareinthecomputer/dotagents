...
**Be careful** - remember best practices. Test driven development. No credentials or identifiers in committed code. Use credential managers (or at least environment variables). Add clear concise comments (that only say what the code does). Never overwrite or edit existing files without reading it first and getting approval, especially for rules, config files, etc. Things like that. 
